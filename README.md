...
A self-writing interactive journal that silently observes your day — *and records what your AI agents did* — and distills it into per-project narrative diaries using a local (or cloud) LLM. Zero manual logging. Nothing leaves your machine unless it has passed the privacy gate, and the optional cloud relay is self-hostable.
...
- **Automation** — set-and-forget toggles, **no admin rights needed**: **Start capturing at logon** (on Windows this drops a launcher in your Startup folder — *not* a scheduled task, so there's no "Access is denied") and **Synthesize the diary every night** at a time you pick. Nightly runs *inside the always-on app*, so it just needs the app to be running — which it is, once start-at-logon is on.
- **Ask your journal** — a search box over your diaries (the `tl ask` Q&A, in the browser). Works without a key (shows the matching diary sections); with a key it answers in prose, grounded strictly in your gated diaries.
...
- **A strict determinism boundary.** Everything is deterministic, stdlib-only, and simulator-tested **except two places**: Phase 1 categorization of genuinely ambiguous intent, and Phase 2 diary/exec-summary prose.
...
sources/ (adapters) ─► privacy/gate ─► timeline (reconcile) ─► data/events/*.jsonl
   ...
   Phase 1  throughlog/categorize.py  signal stack (deterministic), LLM only for ambiguity
                                                                      │
   Phase 2  throughlog/synthesize.py  archive (deterministic) + diary + daily + exec summary (LLM)
```
...
| `tl init [root]` | Auto-discover git repos under a root into a ready-to-edit `projects.json` (paths, git remotes, inferred keywords) — clone to first diary in under 2 minutes. |
| `tl serve` | A stdlib local **dashboard** over the diaries: overview, per-project diary, reconciled timeline, live-capture status. (`tl up` = this + capture + write access.) |
...
| `tl synthesize` | Turn a captured thin-log into per-project diaries + daily + executive summary. |
...
Output lands in `diaries/` (override with `--diaries DIR`) as a three-tier per-project diary: `archive.md` (deterministic, always written) → `journal/<period>.md` (LLM, append-by-day, detail-preserving — the opt-in detailed journal, on by default; `--no-journal` to skip; grouped monthly `<YYYY-MM>` or weekly `<YYYY-Www>` per `synthesis.journal_period`) → `diary.md` (LLM living doc, high-level), plus `daily.md`, `executive_summary.md`, and an optional cross-project weekly/monthly retrospective in `summaries/<period>.md` (`synthesis.summary_cadence`, or build one on demand with `tl summarize --week|--month`).
...
python -m throughlog.synthesize  --smoke                      # live: Phase 2 archive + diary + exec summary
```
