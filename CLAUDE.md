...
- **`config.json`** is gitignored (copy from `config.example.json`). LLM key resolves as inline `llm.api_key` first, else `$OPENROUTER_API_KEY`. Default model `nvidia/nemotron-3-super-120b-a12b:free`. The pipeline runs deterministically with no key (prints a notice, behaves like `--no-llm`).
